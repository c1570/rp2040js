...
describe('USB', () => {
  it('delivers a single scheduled read completion into DPRAM', () => {
    ...
    cpu.usbCtrl.endpointReadDone(0, buffer, 1);
  ...
  it('preserves FIFO order across multiple pending read completions', () => {
    ...
    cpu.usbCtrl.endpointReadDone(0, bufA, 1);
    cpu.usbCtrl.endpointReadDone(0, bufB, 1);
    ...
    cpu.usbCtrl.endpointReadDone(0, bufC, 1);
    cpu.clock.tick(2000);
    ...
    cpu.usbCtrl.endpointReadDone(0, new Uint8Array([0, 0, 0, 0]), 1);
    cpu.clock.tick(2000);
  ...
  it('delivers a single scheduled write completion via onEndpointWrite', () => {
    ...
    cpu.usbCtrl.onEndpointWrite = (endpoint, data) => {
      delivered.push({ endpoint, data: Array.from(data) });
    };
  ...
  it('delivers multiple queued write completions in order on a single fire', () => {
    ...
    cpu.usbCtrl.onEndpointWrite = (_endpoint, data) => {
      delivered.push(Array.from(data));
    };
