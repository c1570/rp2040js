...
int main() {
    ...
    // Start led1 flashing
    blink_pin_forever(pio[0], sm[0], offset[0], PIO_BLINK_LED1_GPIO, 100);

    // Claim the next state machine and start led2 flashing
    pio_sm_claim(pio[0], sm[0] + 1);
    blink_pin_forever(pio[0], sm[0] + 1, offset[0], PIO_BLINK_LED1_GPIO + 1, 90);
    ...
    // Start led3 flashing
    blink_pin_forever(pio[1], sm[1], offset[1], PIO_BLINK_LED3_GPIO, 80);

    // Claim the next state machine and start led4 flashing
    pio_sm_claim(pio[1], sm[1] + 1);
    blink_pin_forever(pio[1], sm[1] + 1, offset[1], PIO_BLINK_LED3_GPIO + 1, 70);
    ...
    int l2 = 0;
    while(1) {
      int l1_new = gpio_get(PIO_BLINK_LED1_GPIO);
      int l2_new = gpio_get(PIO_BLINK_LED3_GPIO);
      if(l1_new != l1) printf("led pin 3: %d\n", l1_new);
      if(l2_new != l2) printf("led pin 32: %d\n", l2_new);
      l1 = l1_new; l2 = l2_new;
    }
}
